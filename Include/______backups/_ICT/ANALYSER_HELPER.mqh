...
//|                                                                  |
//+------------------------------------------------------------------+
bool H4_check_fvg(ANALYSER_FVG &analyser_fvg_, int shift = 0, ENUM_TIMEFRAMES tf = PERIOD_H4)
{
  if (H4_has_bullish_fvg(analyser_fvg_) == true)
  {
    // Print("OK - BULLISH");
  ...
  else if (H4_has_bearish_fvg(analyser_fvg_) == true)
  {
    // Print("OK - BEARISH");
    analyser_fvg_.last_fvg_type_ = FVG_NONE;
    analyser_fvg_.last_fvg_time_ = 0;
  ...
  else
  {
    ...
    // if (time_ == analyser_fvg_.last_fvg_time_ && analyser_fvg_.last_fvg_type_ != FVG_NONE)
    //    {
    //       // mitigated
    //       DebugBreak();
    //       analyser_fvg_.last_fvg_type_ = FVG_NONE;
    ...
    // TODO: CHECK LOOPBACK
    // SEEK 100 LOOPBACK
    LoopbackFVG(analyser_fvg_);
    return false;
  }
}

// BULLISH FVG          - NO CONFIRM
bool H4_has_bullish_fvg(ANALYSER_FVG &analyser_fvg_, int shift = 0, ENUM_TIMEFRAMES tf = PERIOD_H4)
{
  ...
  if (high2Ago < lowNow)
  {
    ...
    analyser_fvg_.atr_ = 0; // on demand copy request
...
// HAS BEARISH FVG
bool H4_has_bearish_fvg(ANALYSER_FVG &analyser_fvg_, int shift = 0, ENUM_TIMEFRAMES tf = PERIOD_H4)
{
  ...
  {
    datetime time_ = iTime(_Symbol, tf, 0 + shift);
    // if(time_ == D'2024.01.16 08:00:00')
    //   {
    //    DebugBreak();
    //   }
...
//|                                                                  |
//+------------------------------------------------------------------+
bool LoopbackFVG(ANALYSER_FVG &analyser_fvg_)
{
  int i = 0;
  ...
  for (i = 0; i < 100; i++)
  {
    bool is_bullish = IsLoopbackFoundBullishFVG(analyser_fvg_, i, PERIOD_H4);
    bool is_bearish = IsLoopbackFoundBearishFVG(analyser_fvg_, i, PERIOD_H4);
  ...
  if (i > 99)
  {
    PrintFormat("NOT FOUND ANALYSE FVG IN LOOPBACK 100");
  }
  return false;
}

// CHECK LOOPBACK FOUND BULLISH FVG
bool IsLoopbackFoundBullishFVG(ANALYSER_FVG &analyser_fvg_, int shift = 0, ENUM_TIMEFRAMES tf = PERIOD_CURRENT)
{
  double high2Ago = iHigh(_Symbol, tf, 2 + shift);
  double lowNow = iLow(_Symbol, tf, 0 + shift);
  ...
  // if(iLow(_Symbol,PERIOD_CURRENT,0) < lowest_)
  //   {
  //    lowest_ = iLow(_Symbol,PERIOD_CURRENT,0);
  //   }
  // if(iHigh(_Symbol,PERIOD_CURRENT,0) > highest_)
  //   {
  //    highest_ = iHigh(_Symbol,PERIOD_CURRENT,0);
  //   }
...
bool IsLoopbackFoundBearishFVG(ANALYSER_FVG &analyser_fvg_, int shift = 0, ENUM_TIMEFRAMES tf = PERIOD_CURRENT)
{
  double low2Ago = iLow(_Symbol, tf, 2 + shift);
  double highNow = iHigh(_Symbol, tf, 0 + shift);
...
bool InitAnalyseATR(ANALYSER_FVG &analyser_fvg_)
{
  ...
  {
    analyser_fvg_.atr_ = ATR_buffer[0];
    // ObjectSetString(0, buttonATR, OBJPROP_TEXT, StringFormat("ATR: %.3f", NormalizeDouble(analyser_fvg.atr_, _Digits)));
  }
